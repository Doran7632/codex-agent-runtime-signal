...
@main
struct CodexAgentRuntimeSignalChecks {
    static func main() throws {
        ...
        print("codex-agent-runtime-signal-checks: ok")
    }

    private static func checkDisplayStateMappings() throws {
        try expect(RuntimeSignal.working.displayState == .active, "working should display as active")
        try expect(RuntimeSignal.thinking.displayState == .active, "thinking should display as active")
        try expect(RuntimeSignal.toolDone.displayState == .active, "tool_done should display as active")
        try expect(RuntimeSignal.done.displayState == .completed, "done should display as completed")
        try expect(RuntimeSignal.notification.displayState == .needsReview, "notification should display as needs_review")
        try expect(RuntimeSignal.permissionRequest.displayState == .permission, "permission_request should display as permission")
        try expect(RuntimeSignal.maxTokens.displayState == .blocked, "max_tokens should display as blocked")
        try expect(RuntimeSignal.off.displayState == .paused, "off should display as paused")
    }
    ...
    private static func checkUnknownSignalIsRejected() throws {
        try expect(RuntimeSignal.normalized("definitely_unknown") == nil, "unknown signal names should be rejected")
    }
