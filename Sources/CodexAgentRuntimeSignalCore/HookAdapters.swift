...
public enum CodexHookAdapter {
    ...
    public static func chooseSignal(eventName: String?, payload: [String: Any]) -> RuntimeSignal {
        if let explicit = firstString(payload, keys: ["signal", "signal_name", "lamp_signal"]),
           let signal = RuntimeSignal.normalized(explicit) {
            return signal
        }

        if let status = firstString(payload, keys: ["status", "state"]) {
            let normalizedStatus = status.trimmingCharacters(in: .whitespacesAndNewlines).lowercased()
            if let signal = RuntimeSignal.normalized(normalizedStatus) {
                return signal
            }
    ...
    public static func agentName(payload: [String: Any], environment: [String: String]) -> String {
        ...
        for key in [
            "CODEX_AGENT_RUNTIME_SIGNAL_AGENT",
            "AGENT_NAME",
            "AGENT_SOURCE",
            "CODEX_SOURCE",
            "CODEX_APP",
...
public enum GenericHookAdapter {
    ...
    public static func chooseSignal(eventName: String?, payload: [String: Any]) -> RuntimeSignal {
        if let explicit = firstString(payload, keys: ["signal", "signal_name", "lamp_signal", "runtime_signal"]),
           let signal = RuntimeSignal.normalized(explicit) {
            return signal
        }

        if let status = firstString(payload, keys: ["status", "state", "phase", "result", "outcome"]) {
            if let signal = RuntimeSignal.normalized(status) {
                return signal
            }
    ...
    public static func sessionKey(
        ...
        for key in [
            "CODEX_AGENT_RUNTIME_SIGNAL_SESSION_ID",
            "AGENT_SESSION_ID",
            "AI_AGENT_SESSION_ID",
            "SESSION_ID",
            "RUN_ID",
    ...
    public static func agentName(payload: [String: Any], environment: [String: String]) -> String {
        ...
        for key in ["CODEX_AGENT_RUNTIME_SIGNAL_AGENT", "AGENT_NAME", "AGENT_SOURCE", "AI_AGENT_NAME"] {
            if let value = environment[key], !value.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
                return value.trimmingCharacters(in: .whitespacesAndNewlines)
            }
        }
...
private func normalizedEventMap(_ events: [String: RuntimeSignal]) -> [String: RuntimeSignal] {
    Dictionary(uniqueKeysWithValues: events.map { key, value in
        (normalizedEventName(key), value)
    })
}

private func signal(for eventName: String?, in events: [String: RuntimeSignal]) -> RuntimeSignal? {
