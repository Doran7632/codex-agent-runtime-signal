...
struct ReleaseInfo: Equatable, Sendable {
    ...
    var clipboardText: String {
        var lines = [
            "codex-agent-runtime-signal \(versionLine)",
            "Release: \(releaseLine)"
        ]
        if let manifestURL {
    ...
    private static func findManifestURL() -> URL? {
        let fileName = "CodexAgentRuntimeSignal-release-manifest.json"
    ...
    private static func findReleaseInfoURL() -> URL? {
        let fileName = "CodexAgentRuntimeSignal-release-info.json"
        ...
        let currentURL = URL(fileURLWithPath: FileManager.default.currentDirectoryPath)
            .standardizedFileURL
            .appendingPathComponent("dist")
            .appendingPathComponent("CodexAgentRuntimeSignal.app")
...
private struct VersionConfiguration {
    ...
    private static func candidateURLs() -> [URL] {
        var urls: [URL] = []
        let resourceFileName = "CodexAgentRuntimeSignal-version.env"
