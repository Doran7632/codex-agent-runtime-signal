...
final class StatusBarController: NSObject, NSMenuDelegate, NSPopoverDelegate, NSWindowDelegate {
    ...
    private lazy var completionBubbleController = CompletionBubbleController(model: model)
    ...
    init(model: MenuBarStatusModel, updater: SparkleUpdaterService) {
        ...
        super.init()
        bind()
        completionBubbleController.start()
        updateStatusItem()
    }

    private func bind() {
        model.$snapshot.sink { [weak self] _ in
            Task { @MainActor in self?.requestStatusItemUpdate() }
        }
        ...
        model.$runtimeSignalAgentScopes.sink { [weak self] _ in
            Task { @MainActor in self?.requestStatusItemUpdate() }
        }
        .store(in: &cancellables)

        model.$runtimeSignalAgentSelectionMode.sink { [weak self] _ in
            Task { @MainActor in self?.requestStatusItemUpdate() }
        }
        .store(in: &cancellables)
    ...
    private func applyAppAppearance() {
        ...
        completionBubbleController.applyAppearance()
    }

    private func requestStatusItemUpdate() {
        guard !isStatusItemUpdateScheduled else { return }
        isStatusItemUpdateScheduled = true

        Task { @MainActor [weak self] in
            guard let self else { return }
            self.isStatusItemUpdateScheduled = false
    ...
    private func updateStatusItem() {
        ...
        let tooltip = model.statusBarTooltip(for: lightSnapshot)
    ...
    private func rebuildNativeStatusMenu(_ menu: NSMenu) {
        ...
        menu.addItem(infoMenuItem(title: "codex-agent-runtime-signal", image: nativeStatusDotImage(for: model.lightSnapshot)))
    ...
    private func nativeVisibleAgentSessions(from snapshot: SignalSnapshot) -> [SessionStatus] {
        ActivityPresentation.visibleSessions(from: snapshot, limit: nil)
    }
    ...
    private func nativeShortEventName(rawEvent: String?, signal: RuntimeSignal) -> String {
    ...
    private func nativeShortSignalName(_ signal: RuntimeSignal) -> String {
    ...
    private func preferredNativeStatusLampColor(for signal: RuntimeSignal) -> SignalLampColor {
        switch signal.displayState {
        case .ready:
            return .green
        case .active:
    ...
    private func preferredActiveLampColor(for signal: RuntimeSignal) -> SignalLampColor {
    ...
    private func showPopover() {
        ...
        popover.contentSize = NSSize(
            ...
            height: max(MenuBarPanelView.minimumPanelHeight, fittingHeight)
        )
    ...
    private func showRecoveryWindow() {
        ...
        window.title = "codex-agent-runtime-signal"
