    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        if DebugLaunchOptions.shouldOpenDebugWindow {
            ...
            DispatchQueue.main.asyncAfter(deadline: .now() + 0.3) {
                RuntimeSignalAppServices.statusBarController.showDebugWindow()
            }
        } else {
...
@MainActor
@main
struct CodexAgentRuntimeSignalApp: App {
    ...
    init() {
        ...
        let sharedModel = RuntimeSignalAppServices.model
        _model = StateObject(wrappedValue: sharedModel)
        _sparkleUpdater = StateObject(wrappedValue: RuntimeSignalAppServices.sparkleUpdater)
        statusBarController = RuntimeSignalAppServices.statusBarController
    }
