...
struct DebugWindowView: View {
    ...
    private enum SettingsDropdownID: Hashable {
        case language
        case theme
        case runtimeSignalAgents
        case thinkingEffect
        case workingEffect
        case doneEffect
        case bubbleCompletionSound
        case bubblePermissionSound
    }
            ...
            settingsSection(model.text("通用", "General")) {
                ...
                settingRow(model.text("新西兰原版模式", "New Zealand original mode")) {
                    settingsSwitch(newZealandTrafficLightModeBinding)
                        .help(model.text(
                            "按新西兰行人红绿灯节奏慢闪。",
                            "Use the original New Zealand pedestrian crossing cadence."
                        ))
                }

                completionBubbleSettings
            }
    ...
    private var isGeneralDropdownExpanded: Bool {
        ...
        case .language, .theme, .bubbleCompletionSound, .bubblePermissionSound:
            return true
        default:
            return false
        }
    ...
    private var bubbleCompletionSoundMenu: some View {
        inlineDropdown(
            id: .bubbleCompletionSound,
            title: model.displayName(for: model.completionBubbleCompletionSound),
            width: settingsPickerWidth
        ) {
            dropdownOptions(width: settingsPickerWidth) {
                ForEach(TaskStatusBubbleSound.allCases, id: \.self) { sound in
                    dropdownOption(
                        model.displayName(for: sound),
                        isSelected: model.completionBubbleCompletionSound == sound,
                        width: settingsPickerWidth
                    ) {
                        model.setCompletionBubbleCompletionSound(sound)
                    }
                }
            }
        }
    ...
    private var bubblePermissionSoundMenu: some View {
        inlineDropdown(
            id: .bubblePermissionSound,
            title: model.displayName(for: model.completionBubblePermissionSound),
            width: settingsPickerWidth
        ) {
            dropdownOptions(width: settingsPickerWidth) {
                ForEach(TaskStatusBubbleSound.allCases, id: \.self) { sound in
                    dropdownOption(
                        model.displayName(for: sound),
                        isSelected: model.completionBubblePermissionSound == sound,
                        width: settingsPickerWidth
                    ) {
                        model.setCompletionBubblePermissionSound(sound)
                    }
                }
            }
        }
        ...
        VStack(alignment: .leading, spacing: 16) {
            settingRow(model.text("开机自启动", "Start at login")) {
                settingsSwitch(launchAtLoginBinding)
                    ...
                    .help(model.text("登录 macOS 后自动打开 codex-agent-runtime-signal", "Open codex-agent-runtime-signal automatically after macOS login"))
            }
    ...
    private var completionBubbleSettings: some View {
        Group {
            settingRow(model.text("任务气泡提醒", "Task status bubble")) {
                settingsSwitch(completionBubbleEnabledBinding)
            }

            if model.isCompletionBubbleEnabled {
                settingRow(model.text("完成提示音", "Completion sound")) {
                    bubbleCompletionSoundMenu
                }
                .zIndex(expandedSettingsDropdown == .bubbleCompletionSound ? 1000 : 0)

                settingControlRow {
                    soundPreviewButton(disabled: model.completionBubbleCompletionSound == .off) {
                        model.previewCompletionBubbleCompletionSound()
                    }
                }

                settingRow(model.text("权限提示音", "Permission sound")) {
                    bubblePermissionSoundMenu
                }
                .zIndex(expandedSettingsDropdown == .bubblePermissionSound ? 1000 : 0)

                settingControlRow {
                    soundPreviewButton(disabled: model.completionBubblePermissionSound == .off) {
                        model.previewCompletionBubblePermissionSound()
                    }
                }
            }
        }
    ...
    private var completionBubbleEnabledBinding: Binding<Bool> {
        Binding(
            get: { model.isCompletionBubbleEnabled },
            set: { model.setCompletionBubbleEnabled($0) }
        )
    }
