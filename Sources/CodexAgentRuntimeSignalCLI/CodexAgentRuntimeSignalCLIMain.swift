...
@main
struct CodexAgentRuntimeSignalCLI {
        ...
        do {
            ...
            case "set", "play":
                ...
                guard let rawSignal = parsed.positionals.first,
                      let signal = RuntimeSignal.normalized(rawSignal)
                else {
            ...
            default:
                if let signal = RuntimeSignal.normalized(command) {
                    let parsed = try parse(Array(arguments.dropFirst()))
                    try requireNoPositionals(parsed)
                    let snapshot: SignalSnapshot
                    if let sessionID = parsed.sessionID {
            ...
            fputs("codex-agent-runtime-signal: \(error.localizedDescription)\n", stderr)
            return 1
        }
    }
}
...
private func printUsage() {
    print(
        ...
        Usage:
          codex-agent-runtime-signal list
          codex-agent-runtime-signal status [--json]
          codex-agent-runtime-signal <signal> [--session <id>] [--agent <name>] [--event <event>] [--json]
          codex-agent-runtime-signal set <signal> [--json]
          codex-agent-runtime-signal session <signal> --session <id> [--json]
          codex-agent-runtime-signal codex-hook [event]
          codex-agent-runtime-signal claude-hook [event]
          codex-agent-runtime-signal agent-hook [event] [--agent <name>] [--session <id>]
          codex-agent-runtime-signal reset [--json]

        Examples:
          codex-agent-runtime-signal idle
          codex-agent-runtime-signal working --session codex-main --agent codex --event PreToolUse
          echo '{"event":"AgentStarted","agent":"local-script","session_id":"local-script-main"}' | codex-agent-runtime-signal agent-hook
          codex-agent-runtime-signal status --json
          codex-agent-runtime-signal permission --session codex-main --event PermissionRequest
...
private func printSignalList() {
    for signal in RuntimeSignal.allCases {
