import Foundation
import XCTest
@testable import CodexAgentRuntimeSignal
@testable import CodexAgentRuntimeSignalCore
@testable import CodexAgentRuntimeSignalUI

final class CodexAgentRuntimeSignalCoreTests: XCTestCase {
    func testSignalNormalizationAcceptsHumanInputVariants() {
        XCTAssert(RuntimeSignal.normalized("tool-done") == .toolDone)
        XCTAssert(RuntimeSignal.normalized(" session start ") == .sessionStart)
        XCTAssert(RuntimeSignal.normalized("PERMISSION") == .permission)
        XCTAssert(RuntimeSignal.normalized("PermissionRequest") == .permissionRequest)
        XCTAssert(RuntimeSignal.normalized("notification") == .notification)
        XCTAssert(RuntimeSignal.normalized("max-tokens") == .maxTokens)
    }
    ...
    func testDisplayStateMappingMatchesV2Language() {
        XCTAssert(RuntimeSignal.idle.displayState == .ready)
        XCTAssert(RuntimeSignal.thinking.displayState == .active)
        XCTAssert(RuntimeSignal.working.displayState == .active)
        XCTAssert(RuntimeSignal.toolDone.displayState == .active)
        XCTAssert(RuntimeSignal.subagentStart.displayState == .active)
        XCTAssert(RuntimeSignal.done.displayState == .completed)
        XCTAssert(RuntimeSignal.attention.displayState == .needsReview)
        XCTAssert(RuntimeSignal.notification.displayState == .needsReview)
        XCTAssert(RuntimeSignal.permission.displayState == .permission)
        XCTAssert(RuntimeSignal.permissionRequest.displayState == .permission)
        XCTAssert(RuntimeSignal.blocked.displayState == .blocked)
        XCTAssert(RuntimeSignal.failure.displayState == .blocked)
        XCTAssert(RuntimeSignal.maxTokens.displayState == .blocked)
        XCTAssert(RuntimeSignal.stale.displayState == .stale)
        XCTAssert(RuntimeSignal.off.displayState == .paused)
    }
    ...
    func testDefaultEventLimitSupportsFiftyRecentEvents() {
        ...
        XCTAssertEqual(
            SignalStateStore.defaultEventLimit(environment: ["CODEX_AGENT_RUNTIME_SIGNAL_EVENT_LIMIT": "12"]),
            12
        )
    }
    ...
    func testRedundantReadyAndActiveRefreshesAreCoalesced() throws {
        let fixture = try makeTemporaryStore(eventLimit: 4)
        defer { try? FileManager.default.removeItem(at: fixture.directory) }
        let firstDate = Date(timeIntervalSince1970: 1_780_358_400)
        let redundantDate = firstDate.addingTimeInterval(10)
        let refreshDate = firstDate.addingTimeInterval(75)
        ...
        let redundantSnapshot = try fixture.store.applySessionSignal(
            .working,
            sessionID: "codex-main",
            agent: "codex",
            lastEvent: "DesktopActivityHeartbeat",
            updatedAt: redundantDate
        )

        XCTAssertEqual(redundantSnapshot.sessions.first?.updatedAt, firstDate)
        XCTAssertEqual(redundantSnapshot.updatedAt, firstDate)
        XCTAssertEqual(redundantSnapshot.recentEvents.map(\.updatedAt), [firstDate])

        let refreshedSnapshot = try fixture.store.applySessionSignal(
            .working,
            sessionID: "codex-main",
            agent: "codex",
            lastEvent: "DesktopActivityHeartbeat",
        ...
        XCTAssertEqual(refreshedSnapshot.sessions.first?.updatedAt, refreshDate)
        XCTAssertEqual(refreshedSnapshot.updatedAt, refreshDate)
        XCTAssertEqual(refreshedSnapshot.recentEvents.map(\.updatedAt), [refreshDate, firstDate])
    }
    ...
    func testDefaultStateFileURLTrimsBlankEnvironmentValues() {
        let explicitFallback = SignalStateStore.defaultStateFileURL(
            environment: [
                "CODEX_AGENT_RUNTIME_SIGNAL_STATE_FILE": "   ",
                "CODEX_AGENT_RUNTIME_SIGNAL_STATE_DIR": "  /tmp/trimmed-codex-agent-runtime-signal  "
            ]
        )
        XCTAssertEqual(explicitFallback.path, "/tmp/trimmed-codex-agent-runtime-signal/status.json")

        let defaultFallback = SignalStateStore.defaultStateFileURL(
            environment: [
                "CODEX_AGENT_RUNTIME_SIGNAL_STATE_FILE": "\n\t",
                "CODEX_AGENT_RUNTIME_SIGNAL_STATE_DIR": "  ",
                "RUNTIME_SIGNAL_STATE_DIR": "\n"
            ]
        )
        XCTAssertEqual(defaultFallback.path, "/tmp/codex-agent-runtime-signal/status.json")
    }
    ...
    func testGitHubReleaseUpdateCheckerDecodesLatestRelease() throws {
        let data = Data(
            ...
            {
              "tag_name": "v1.2.0",
              "html_url": "https://github.com/guan-ops/codex-agent-runtime-signal/releases/tag/v1.2.0",
              "assets": [
                {
                  "name": "CodexAgentRuntimeSignal-local.dmg",
                  "browser_download_url": "https://github.com/guan-ops/codex-agent-runtime-signal/releases/download/v1.2.0/CodexAgentRuntimeSignal-local.dmg"
                }
              ]
            }
            """.utf8
        ...
        XCTAssertEqual(release.preferredDownloadURL?.lastPathComponent, "CodexAgentRuntimeSignal-local.dmg")
    }
    ...
    func testGitHubReleaseUpdateCheckerFallsBackToReleasePageWhenAPIRateLimited() async throws {
        ...
        let releasePageURL = URL(string: "https://github.com/guan-ops/codex-agent-runtime-signal/releases/tag/v1.2.0")!
    ...
    func testActivityPresentationKeepsCodexEntrypointsSeparate() {
        ...
        let snapshot = SignalSnapshot(
            ...
            recentEvents: [],
            stateFileURL: URL(fileURLWithPath: "/tmp/codex-agent-runtime-signal/status.json"),
            updatedAt: now
        )
    ...
    func testActivityPresentationKeepsMultipleSessionsFromSameSource() {
        ...
        let visible = ActivityPresentation.visibleSessions(
            from: [
                SessionStatus(
                    sessionID: "codex-cli:first-thread",
                    signal: .working,
                    updatedAt: now,
                    agent: "codex-cli",
                    lastEvent: "DesktopActivityHeartbeat"
                ...
                SessionStatus(
                    sessionID: "codex-cli:second-thread",
                    signal: .thinking,
                    updatedAt: now.addingTimeInterval(-1),
                    agent: "codex-cli",
                    lastEvent: "DesktopThinking"
        ...
        XCTAssertEqual(Set(visible.map(\.sessionID)), [
            "codex-cli:first-thread",
            "codex-cli:second-thread"
        ])
    }
    ...
    func testActivityPresentationCollapsesMultipleRowsForSameCodexThread() {
        ...
        let visible = ActivityPresentation.visibleSessions(
            from: [
                SessionStatus(
                    sessionID: "codex-vscode:\(threadID)",
                    signal: .thinking,
                    updatedAt: now,
                    agent: "codex-vscode",
                    lastEvent: "UserPromptSubmit"
                ...
                SessionStatus(
                    sessionID: "codex-desktop:\(threadID)",
                    signal: .working,
                    updatedAt: now,
                    agent: "codex-desktop",
                    lastEvent: "DesktopToolCall:exec_command"
                ...
                SessionStatus(
                    sessionID: "codex-cli:another-thread",
                    signal: .working,
                    updatedAt: now,
                    agent: "codex-cli",
                    lastEvent: "DesktopMessage"
        ...
        XCTAssertEqual(
            visible.filter { ActivityPresentation.activitySessionIdentityKey(for: $0) == "codex:thread:\(threadID)" }.count,
            1
        )
    ...
    @MainActor
    func testActivitySessionLineUsesCodexThreadName() {
        let model = MenuBarStatusModel(
            codexThreadNameResolver: StaticCodexThreadNameResolver(namesBySessionID: [
                "codex-idea:thread-a": "Offset Explorer帮我安装这个软件"
            ])
        )
        model.appLanguage = .zhHans
        let session = SessionStatus(
            sessionID: "codex-idea:thread-a",
            signal: .working,
            updatedAt: Date(),
            agent: "codex-idea",
            lastEvent: "DesktopToolCall:exec_command"
        ...
        XCTAssertEqual(
            ...
            "IDEA - 运行中 - (Offset Explorer帮我安装这个软件)"
        )
    }

    @MainActor
    func testActivitySessionLineDoesNotExposeCodexSessionIDWhenNameIsMissing() {
        let model = MenuBarStatusModel(
            codexThreadNameResolver: StaticCodexThreadNameResolver(namesBySessionID: [:])
        )
        model.appLanguage = .zhHans
        let session = SessionStatus(
            sessionID: "codex-cli:019edad4-7277-7da2-bc96-9d00ffb7fa0b",
            signal: .working,
            updatedAt: Date(),
            agent: "codex-cli",
            lastEvent: "DesktopActivityHeartbeat"
    ...
    @MainActor
    func testActivitySessionLineUsesCodexRolloutMetadataWhenIndexIsMissing() throws {
        let fixture = try makeTemporaryCodexSessionsRoot()
        defer { try? FileManager.default.removeItem(at: fixture.directory) }
        let threadID = "019ecbeb-41d4-77f2-ab20-725f014b560c"
        let sessionFile = fixture.sessionsRoot
            .appendingPathComponent("2026/06/15", isDirectory: true)
            .appendingPathComponent("rollout-2026-06-15T23-34-13-\(threadID).jsonl")
        try FileManager.default.createDirectory(
            at: sessionFile.deletingLastPathComponent(),
            withIntermediateDirectories: true
        )
        try [
            #"{"type":"session_meta","payload":{"id":"\#(threadID)","cwd":"/Users/doranxavier","originator":"codex-tui","source":"cli"}}"#,
            ##"{"type":"response_item","payload":{"type":"message","role":"user","content":[{"type":"input_text","text":"# AGENTS.md instructions for /Users/doranxavier\n\n<INSTRUCTIONS>...</INSTRUCTIONS>"}]}}"##,
            ##"{"type":"event_msg","payload":{"type":"user_message","message":"# Instructions (read first)\n\n## Security: prompt injection resistance"}}"##,
            #"{"type":"event_msg","payload":{"type":"user_message","message":"帮我检测一下为什么现在电脑风扇开始转了，哪个软件开始耗能变高了"}}"#
        ].joined(separator: "\n")
            .appending("\n")
            .write(to: sessionFile, atomically: true, encoding: .utf8)

        let resolver = CodexThreadNameIndex(
            indexURL: fixture.directory.appendingPathComponent("missing-session-index.jsonl"),
            sessionsRootURL: fixture.sessionsRoot
        )
        let model = MenuBarStatusModel(codexThreadNameResolver: resolver)
        model.appLanguage = .zhHans
        let session = SessionStatus(
            sessionID: "codex-cli:\(threadID)",
            signal: .working,
        ...
        XCTAssertEqual(
            model.activitySessionLine(for: session),
            "桌面终端 - 运行中 - (帮我检测一下为什么现在电脑风扇开始转了，哪个软件开始耗能变高了)"
        )
    }

    @MainActor
    func testActivitySessionLineShowsHostApplicationFromCodexRolloutCWD() throws {
        let fixture = try makeTemporaryCodexSessionsRoot()
        defer { try? FileManager.default.removeItem(at: fixture.directory) }
        let threadID = "019ede79-21dd-77a3-8e96-557de4f7a7ee"
        let sessionFile = fixture.sessionsRoot
            .appendingPathComponent("2026/06/19", isDirectory: true)
            .appendingPathComponent("rollout-2026-06-19T14-02-21-\(threadID).jsonl")
        try FileManager.default.createDirectory(
            at: sessionFile.deletingLastPathComponent(),
            withIntermediateDirectories: true
        )
        try [
            #"{"type":"session_meta","payload":{"id":"\#(threadID)","cwd":"/Users/doranxavier/Library/Application Support/Open Design/namespaces/release-stable/data/projects/project-id","originator":"codex_exec","source":"exec"}}"#,
            #"{"type":"event_msg","payload":{"type":"user_message","message":"帮我生成一套输入法皮肤"}}"#
        ].joined(separator: "\n")
            .appending("\n")
            .write(to: sessionFile, atomically: true, encoding: .utf8)
        ...
        XCTAssertEqual(
            model.activitySessionLine(for: session),
            "OpenDesign - 运行中 - (帮我生成一套输入法皮肤)"
        )
    }

    @MainActor
    func testActivitySessionLineExtractsOpenDesignEmbeddedUserRequest() throws {
        let fixture = try makeTemporaryCodexSessionsRoot()
        defer { try? FileManager.default.removeItem(at: fixture.directory) }
        let threadID = "019edf90-01dd-71f1-b2fb-47bc10a6e1dc"
        let sessionFile = fixture.sessionsRoot
            .appendingPathComponent("2026/06/19", isDirectory: true)
            .appendingPathComponent("rollout-2026-06-19T19-06-57-\(threadID).jsonl")
        try FileManager.default.createDirectory(
            at: sessionFile.deletingLastPathComponent(),
            withIntermediateDirectories: true
        )
        try [
            #"{"type":"session_meta","payload":{"id":"\#(threadID)","cwd":"/Users/doranxavier/Library/Application Support/Open Design/namespaces/release-stable/data/projects/project-id","originator":"codex_exec","source":"exec"}}"#,
            ##"{"type":"event_msg","payload":{"type":"user_message","message":"# OpenDesign context\nLarge generated context...\n\n## user\n## context warning\nOpen Design detected a large active file.\nKeep this turn compact.\n\n# User request\n帮我把登录页改成深色模式\n\n# Project files\n..."}}"##
        ].joined(separator: "\n")
            .appending("\n")
            .write(to: sessionFile, atomically: true, encoding: .utf8)
        ...
        XCTAssertEqual(
            model.activitySessionLine(for: session),
            "OpenDesign - 运行中 - (帮我把登录页改成深色模式)"
        )
    }

    func testOpenCodexSessionFromLsofRolloutFileUsesRealThreadAndHostApp() {
        let now = Date(timeIntervalSince1970: 1_000)
        let threadID = "019e9055-6952-7e50-b1fb-183473424e0b"
        let sessions = CodexPlatformPresenceMonitor.detectOpenCodexSessions(
            processes: [
                ...
                CodexPlatformPresenceMonitor.RunningProcessInfo(
                    ...
                    ppid: 7054,
                    command: "/bin/zsh",
                    arguments: "/bin/zsh --login -i"
                ),
                CodexPlatformPresenceMonitor.RunningProcessInfo(
                    pid: 13320,
                    ppid: 12763,
                    command: "node",
                    arguments: "node /Users/me/.nvm/versions/node/v24.2.0/bin/codex resume"
                ),
                CodexPlatformPresenceMonitor.RunningProcessInfo(
                    pid: 13321,
                    ppid: 13320,
                    command: "/Users/me/.nvm/versions/node/v24.2.0/lib/node_modules/@openai/codex/node_modules/@openai/codex-darwin-arm64/vendor/aarch64-apple-darwin/bin/codex",
                    arguments: "resume"
                )
            ],
            openRolloutFilesByPID: [
                13321: "/Users/me/.codex/sessions/2026/06/04/rollout-2026-06-04T09-52-57-\(threadID).jsonl"
            ],
            now: now
        )

        XCTAssertEqual(sessions.map(\.sessionID), ["codex-idea:\(threadID)"])
        XCTAssertEqual(sessions.map(\.signal), [.idle])
        XCTAssertEqual(sessions.map(\.lastEvent), ["CodexSessionOpen"])
    }

    func testOpenCodexSessionFromVSCodeAppServerRolloutFileUsesRealThread() {
        let now = Date(timeIntervalSince1970: 1_000)
        let threadID = "019edad4-7277-7da2-bc96-9d00ffb7fa0b"
        let sessions = CodexPlatformPresenceMonitor.detectOpenCodexSessions(
            processes: [
                CodexPlatformPresenceMonitor.RunningProcessInfo(
                    ...
                    arguments: "/Applications/Visual Studio Code.app/Contents/MacOS/Code"
                ),
                CodexPlatformPresenceMonitor.RunningProcessInfo(
                    pid: 2374,
                    ppid: 604,
                    command: "/Applications/Visual Studio Code.app/Contents/Frameworks/Code Helper (Plugin).app/Contents/MacOS/Code Helper (Plugin)",
                    arguments: "--type=utility"
                ),
                CodexPlatformPresenceMonitor.RunningProcessInfo(
                    pid: 11746,
                    ppid: 2374,
                    command: "/Users/me/.vscode/extensions/openai.chatgpt-26.601.21317-darwin-arm64/bin/macos-aarch64/codex",
                    arguments: "app-server --analytics-default-enabled"
                )
            ],
            openRolloutFilesByPID: [
                11746: "/Users/me/.codex/sessions/2026/06/18/rollout-2026-06-18T21-03-36-\(threadID).jsonl"
            ],
            now: now
        )

        XCTAssertEqual(sessions.map(\.sessionID), ["codex-vscode:\(threadID)"])
        XCTAssertEqual(sessions.map(\.signal), [.idle])
        XCTAssertEqual(sessions.map(\.lastEvent), ["CodexSessionOpen"])
    }

    func testOpenCodexSessionFromObsidianACPUsesRealThreadAndHostApp() {
        let now = Date(timeIntervalSince1970: 1_000)
        let threadID = "019e8cb0-023f-71e0-9c98-f24e1ba8d779"
        let sessions = CodexPlatformPresenceMonitor.detectOpenCodexSessions(
            processes: [
                CodexPlatformPresenceMonitor.RunningProcessInfo(
                    pid: 42150,
                    ...
                    arguments: "/Applications/Obsidian.app/Contents/MacOS/Obsidian"
                ),
                CodexPlatformPresenceMonitor.RunningProcessInfo(
                    pid: 42153,
                    ppid: 42150,
                    command: "/Applications/Obsidian.app/Contents/Frameworks/Obsidian Helper (Renderer).app/Contents/MacOS/Obsidian Helper (Renderer)",
                    arguments: "--type=renderer --user-data-dir=/Users/me/Library/Application Support/obsidian"
                ),
                CodexPlatformPresenceMonitor.RunningProcessInfo(
                    pid: 42162,
                    ppid: 42153,
                    command: "/Users/me/.nvm/versions/node/v24.2.0/bin/node",
                    arguments: "/Users/me/.nvm/versions/node/v24.2.0/lib/node_modules/@zed-industries/codex-acp/bin/codex-acp.js"
                ),
                CodexPlatformPresenceMonitor.RunningProcessInfo(
                    pid: 42188,
                    ppid: 42162,
                    command: "/Users/me/.nvm/versions/node/v24.2.0/lib/node_modules/@zed-industries/codex-acp/node_modules/@zed-industries/codex-acp-darwin-arm64/bin/codex-acp",
                    arguments: ""
                )
            ],
            openRolloutFilesByPID: [
                42188: "/Users/me/.codex/sessions/2026/06/03/rollout-2026-06-03T16-53-25-\(threadID).jsonl"
            ],
            now: now
        )

        XCTAssertEqual(sessions.map(\.sessionID), ["codex-obsidian:\(threadID)"])
        XCTAssertEqual(sessions.map(\.signal), [.idle])
        XCTAssertEqual(sessions.map(\.lastEvent), ["CodexSessionOpen"])
    }

    @MainActor
    func testActivitySessionLineUsesObsidianHostApplication() {
        let model = MenuBarStatusModel(
            codexThreadNameResolver: StaticCodexThreadNameResolver(namesBySessionID: [
                "codex-obsidian:thread-a": "整理 Obsidian 笔记"
            ])
        )
        model.appLanguage = .zhHans
        let session = SessionStatus(
            sessionID: "codex-obsidian:thread-a",
            signal: .idle,
            updatedAt: Date(),
            agent: "codex-obsidian",
    ...
    func testComputerUseTurnEndedProcessDoesNotCreateOpenCodexSession() {
        let now = Date(timeIntervalSince1970: 1_000)
        let sessions = CodexPlatformPresenceMonitor.detectSessions(
            ...
            processes: [
                CodexPlatformPresenceMonitor.RunningProcessInfo(
                    pid: 10,
                    ppid: 1,
                    command: "/Users/me/.codex/computer-use/Codex Computer Use.app/Contents/SharedSupport/SkyComputerUseClient.app/Contents/MacOS/SkyComputerUseClient",
                    arguments: ##"turn-ended {"type":"agent-turn-complete","thread-id":"old-open-design-thread","cwd":"/Users/me/Library/Application Support/Open Design/namespaces/release-stable/data/projects/old","client":"codex_exec","input-messages":["# Instructions (read first)","做一个设计稿"]}"##
                ),
                CodexPlatformPresenceMonitor.RunningProcessInfo(
                    pid: 20,
                    ppid: 1,
                    command: "/Users/me/.codex/computer-use/Codex Computer Use.app/Contents/SharedSupport/SkyComputerUseClient.app/Contents/MacOS/SkyComputerUseClient",
                    arguments: ##"turn-ended {"type":"agent-turn-complete","thread-id":"latest-open-design-thread","cwd":"/Users/me/Library/Application Support/Open Design/namespaces/release-stable/data/projects/latest","client":"codex_exec","input-messages":["# Instructions (read first)","继续"]}"##
                ),
                CodexPlatformPresenceMonitor.RunningProcessInfo(
                    pid: 30,
                    ppid: 1,
                    command: "/Users/me/.codex/computer-use/Codex Computer Use.app/Contents/SharedSupport/SkyComputerUseClient.app/Contents/MacOS/SkyComputerUseClient",
                    arguments: #"turn-ended {"type":"agent-turn-complete","thread-id":"memory-thread","cwd":"/Applications/Open Design.app/Contents/Resources/app/prebundled","client":"codex_exec","input-messages":["You are a memory extractor for a personal AI design assistant."]}"#
                )
            ],
            now: now
        )

        XCTAssertFalse(sessions.contains { $0.sessionID == "codex-cli:latest-open-design-thread" })
        XCTAssertFalse(sessions.contains { $0.sessionID == "codex-cli:old-open-design-thread" })
    }

    func testOpenCodexSessionStaysVisibleWhenIdle() {
        let now = Date(timeIntervalSince1970: 1_000)
        let visible = ActivityPresentation.visibleSessions(
            from: [
                SessionStatus(
                    sessionID: "codex-idea:019e9055-6952-7e50-b1fb-183473424e0b",
                    signal: .idle,
                    updatedAt: now,
                    agent: "codex-idea",
                    lastEvent: "CodexSessionOpen"
        ...
        XCTAssertEqual(visible.map(\.sessionID), ["codex-idea:019e9055-6952-7e50-b1fb-183473424e0b"])
    }

    func testCodexResumeIndexSessionDoesNotStayVisibleWhenIdle() {
        let now = Date(timeIntervalSince1970: 1_000)
        let visible = ActivityPresentation.visibleSessions(
            from: [
                SessionStatus(
                    sessionID: "codex-cli:019ede79-21dd-77a3-8e96-557de4f7a7ee",
                    signal: .idle,
                    updatedAt: now.addingTimeInterval(-86_400),
                    agent: "codex-cli",
                    lastEvent: "CodexResumeIndex"
                )
            ],
            now: now
        )
    ...
    func testActivityPresentationKeepsTiedOpenCodexSessionsInStableOrder() {
        let now = Date(timeIntervalSince1970: 1_000)
        let sessionIDs = [
            "codex-idea:019e0000-0000-7000-8000-000000000004",
            "codex-idea:019e0000-0000-7000-8000-000000000001",
            "codex-idea:019e0000-0000-7000-8000-000000000003",
            "codex-idea:019e0000-0000-7000-8000-000000000002"
        ]
        let visible = ActivityPresentation.visibleSessions(
            from: sessionIDs.map { sessionID in
                SessionStatus(
                    sessionID: sessionID,
                    signal: .idle,
                    updatedAt: now,
    ...
    func testCompletionBubbleTrackerNotifiesOnceUntilSessionRunsAgain() {
        let tracker = CompletionBubbleEventTracker()
        let now = Date(timeIntervalSince1970: 1_000)
        let running = CompletionBubbleCandidate(
            identityKey: "codex:thread:task",
            signal: .working,
            updatedAt: now,
            sourceApplication: "IDEA",
            sessionName: "申万二期迁移功能七"
        )
        let done = CompletionBubbleCandidate(
            identityKey: "codex:thread:task",
            signal: .done,
        ...
        let replayedDone = CompletionBubbleCandidate(
            identityKey: "codex:thread:task",
            signal: .done,
        ...
        let rerun = CompletionBubbleCandidate(
            identityKey: "codex:thread:task",
            signal: .thinking,
        ...
        let doneAgain = CompletionBubbleCandidate(
            identityKey: "codex:thread:task",
            signal: .done,
        ...
        XCTAssertEqual(tracker.notifications(from: [running, done]).map(\.updatedAt), [done.updatedAt])
        XCTAssertTrue(tracker.notifications(from: [done, replayedDone]).isEmpty)
        XCTAssertTrue(tracker.notifications(from: [replayedDone, rerun]).isEmpty)
        XCTAssertEqual(tracker.notifications(from: [rerun, doneAgain]).map(\.updatedAt), [doneAgain.updatedAt])
    }

    func testCompletionBubbleTrackerIgnoresAbortedCompletionEvents() {
        let tracker = CompletionBubbleEventTracker()
        let now = Date(timeIntervalSince1970: 1_000)
        let running = CompletionBubbleCandidate(
            identityKey: "codex:thread:task",
            signal: .working,
            updatedAt: now,
            sourceApplication: "桌面终端",
            sessionName: "解释斐波那契数列",
            event: "DesktopThinking"
        )
        let aborted = CompletionBubbleCandidate(
            identityKey: "codex:thread:task",
            signal: .done,
            updatedAt: now.addingTimeInterval(5),
            sourceApplication: "桌面终端",
        ...
        let completed = CompletionBubbleCandidate(
            identityKey: "codex:thread:task",
            signal: .done,
        ...
        XCTAssertTrue(tracker.notifications(from: [running, aborted]).isEmpty)
        XCTAssertEqual(tracker.notifications(from: [running, completed]).map(\.updatedAt), [completed.updatedAt])
    }

    func testCompletionBubbleTrackerNotifiesForPermissionOnceUntilSessionRunsAgain() {
        let tracker = CompletionBubbleEventTracker()
        let now = Date(timeIntervalSince1970: 1_000)
        let running = CompletionBubbleCandidate(
            identityKey: "codex:thread:task",
            ...
            updatedAt: now,
            sourceApplication: "IDEA",
            sessionName: "Offset Explorer帮我安装这个软件",
            event: "PreToolUse"
        )
        let permission = CompletionBubbleCandidate(
            identityKey: "codex:thread:task",
        ...
        let replayedPermission = CompletionBubbleCandidate(
            identityKey: "codex:thread:task",
            signal: .permissionRequest,
        ...
        let permissionAgain = CompletionBubbleCandidate(
            identityKey: "codex:thread:task",
            signal: .permission,
            updatedAt: now.addingTimeInterval(40),
            sourceApplication: "IDEA",
        ...
        let firstNotifications = tracker.notifications(from: [running, permission])
        XCTAssertEqual(firstNotifications.map(\.updatedAt), [permission.updatedAt])
        XCTAssertEqual(firstNotifications.first?.notificationKind, .permission)
        XCTAssertTrue(tracker.notifications(from: [permission, replayedPermission]).isEmpty)
        XCTAssertTrue(tracker.notifications(from: [replayedPermission, rerun]).isEmpty)
        XCTAssertEqual(
            tracker.notifications(from: [rerun, permissionAgain]).map(\.updatedAt),
            [permissionAgain.updatedAt]
        )
    }

    @MainActor
    func testOpenCodexSessionIsRetainedAcrossTransientPresenceMiss() {
        let model = MenuBarStatusModel(
            codexPlatformPresenceMonitor: CodexPlatformPresenceMonitor(processScanInterval: 3_600)
        )
        model.clearRetainedOpenCodexSessions()
        let now = Date(timeIntervalSince1970: 1_000)
        let session = SessionStatus(
            sessionID: "codex-cli:019ecbeb-41d4-77f2-ab20-725f014b560c",
            signal: .idle,
            updatedAt: now,
            agent: "codex-cli",
            lastEvent: "CodexSessionOpen"
        ...
        XCTAssertEqual(
            model.stabilizedPlatformPresenceSessions([session], now: now).map(\.sessionID),
            [session.sessionID]
        )
        XCTAssertEqual(
    ...
    @MainActor
    func testOpenTerminalCodexSessionIsNotSuppressedByAnotherRunningTerminalThread() throws {
        let fixture = try makeTemporaryStore(sessionTTLSeconds: 300)
        defer { try? FileManager.default.removeItem(at: fixture.directory) }

        let now = Date()
        let runningThreadID = "019edad4-7277-7da2-bc96-9d00ffb7fa0b"
        let idleThreadID = "019ecbeb-41d4-77f2-ab20-725f014b560c"

        _ = try fixture.store.applySessionSignal(
            .working,
            sessionID: "codex-cli:\(runningThreadID)",
            agent: "codex-cli",
            lastEvent: "PreToolUse",
            updatedAt: now
        )
        ...
        let model = MenuBarStatusModel(
            ...
            codexPlatformPresenceMonitor: CodexPlatformPresenceMonitor(processScanInterval: 3_600),
            codexThreadNameResolver: StaticCodexThreadNameResolver(namesBySessionID: [
                "codex-cli:\(runningThreadID)": "正在运行的终端会话",
                "codex-cli:\(idleThreadID)": "打开但空闲的终端会话"
            ])
        )
        model.appLanguage = .zhHans
        model.replaceDesktopAppSessionsForTesting([
            SessionStatus(
                sessionID: "codex-cli:\(idleThreadID)",
                signal: .idle,
                updatedAt: now,
                agent: "codex-cli",
                lastEvent: "CodexSessionOpen"
        ...
        XCTAssertTrue(visible.contains { $0.sessionID == "codex-cli:\(runningThreadID)" })
        XCTAssertTrue(visible.contains { $0.sessionID == "codex-cli:\(idleThreadID)" })
        XCTAssertEqual(
    ...
    @MainActor
    func testActivitySnapshotDoesNotIncludeClosedCodexResumeIndexSessions() throws {
        let fixture = try makeTemporaryStore(sessionTTLSeconds: 60)
        defer { try? FileManager.default.removeItem(at: fixture.directory) }
        let now = Date(timeIntervalSince1970: 1_000)
        let model = MenuBarStatusModel(
            store: fixture.store,
            ...
            codexThreadNameResolver: StaticCodexThreadNameResolver(
                namesBySessionID: [
                    "codex-cli:open-design-thread": "OpenDesign会话",
                    "codex-idea:idea-thread": "IDEA会话"
                ],
                indexedSessions: [
                    CodexIndexedSession(
                        threadID: "open-design-thread",
                        threadName: "OpenDesign会话",
                        updatedAt: now,
                        hostApplicationName: "OpenDesign"
                    ),
                    CodexIndexedSession(
                        threadID: "idea-thread",
                        threadName: "IDEA会话",
                        updatedAt: now.addingTimeInterval(-1),
                        hostApplicationName: "IDEA"
                    )
                ]
            )
        )
        ...
        let visible = ActivityPresentation.visibleSessions(from: model.activitySnapshot, now: now, limit: nil)
        XCTAssertFalse(visible.contains { $0.sessionID == "codex-cli:open-design-thread" })
        XCTAssertFalse(visible.contains { $0.sessionID == "codex-idea:idea-thread" })
    }

    @MainActor
    func testActivitySnapshotDoesNotIncludeRolloutOnlyKnownCodexSessions() throws {
        let fixture = try makeTemporaryStore(sessionTTLSeconds: 60)
        defer { try? FileManager.default.removeItem(at: fixture.directory) }
        let sessionsRoot = fixture.directory.appendingPathComponent("codex-sessions", isDirectory: true)
        let threadID = "019edf90-01dd-71f1-b2fb-47bc10a6e1dc"
        let sessionFile = sessionsRoot
            .appendingPathComponent("2026/06/19", isDirectory: true)
            .appendingPathComponent("rollout-2026-06-19T19-06-57-\(threadID).jsonl")
        try FileManager.default.createDirectory(
            at: sessionFile.deletingLastPathComponent(),
        ...
        let model = MenuBarStatusModel(
            store: fixture.store,
            codexPlatformPresenceMonitor: CodexPlatformPresenceMonitor(processScanInterval: 3_600),
            codexThreadNameResolver: resolver
        )
        model.appLanguage = .zhHans

        let visible = ActivityPresentation.visibleSessions(from: model.activitySnapshot, limit: nil)
        XCTAssertFalse(visible.contains { $0.sessionID == "codex-cli:\(threadID)" })
        XCTAssertFalse(visible.contains { model.activitySessionLine(for: $0) == "OpenDesign - 空闲 - (帮我生成一套输入法皮肤)" })
    }
    ...
    func testActivityPresentationHidesTerminalPresenceWhenThereIsNoActiveCodexThread() {
        let now = Date(timeIntervalSince1970: 1_000)
        let visible = ActivityPresentation.visibleSessions(
            from: [
                SessionStatus(
    ...
    func testRecentDesktopActivityOverridesPresenceOnlySession() throws {
        let savedDefaults = clearRuntimeSignalSelectionDefaults()
        defer { restoreRuntimeSignalSelectionDefaults(savedDefaults) }
        let fixture = try makeTemporaryStore()
        defer { try? FileManager.default.removeItem(at: fixture.directory) }
        let now = Date()
        ...
        model.setRuntimeSignalAgentScopes([.codexDesktop])
    ...
    @MainActor
    func testRuntimeSignalAutomaticallyFollowsHighestPriorityActiveSource() throws {
        let savedDefaults = clearRuntimeSignalSelectionDefaults()
        defer { restoreRuntimeSignalSelectionDefaults(savedDefaults) }
        let fixture = try makeTemporaryStore()
        defer { try? FileManager.default.removeItem(at: fixture.directory) }
        ...
        XCTAssertEqual(model.runtimeSignalAgentSelectionMode, .following)
        XCTAssertEqual(model.displayRuntimeSignalAgentScopes, [.claudeCode])
    ...
    @MainActor
    func testManualRuntimeSignalSelectionAggregatesMultipleSelectedSourcesOnly() throws {
        let savedDefaults = clearRuntimeSignalSelectionDefaults()
        defer { restoreRuntimeSignalSelectionDefaults(savedDefaults) }
        let fixture = try makeTemporaryStore()
        defer { try? FileManager.default.removeItem(at: fixture.directory) }
        ...
        let model = MenuBarStatusModel(store: fixture.store)
        model.setRuntimeSignalAgentScopes([.codexDesktop, .codexCLI])

        XCTAssertEqual(model.runtimeSignalAgentSelectionMode, .manual)
        XCTAssertEqual(model.displayRuntimeSignalAgentScopes, [.codexDesktop, .codexCLI])
        XCTAssertEqual(model.displaySnapshot.aggregate, .working)
        XCTAssertEqual(
    ...
    @MainActor
    func testManualRuntimeSignalSelectionShowsHintWithoutSwitchingWhenOtherAgentsRun() throws {
        let savedDefaults = clearRuntimeSignalSelectionDefaults()
        defer { restoreRuntimeSignalSelectionDefaults(savedDefaults) }
        let fixture = try makeTemporaryStore()
        defer { try? FileManager.default.removeItem(at: fixture.directory) }
        ...
        let model = MenuBarStatusModel(store: fixture.store)
        model.setRuntimeSignalAgentScopes([.localScript])

        XCTAssertEqual(model.runtimeSignalAgentSelectionMode, .manual)
        XCTAssertEqual(model.displayRuntimeSignalAgentScopes, [.localScript])
        XCTAssertEqual(model.displaySnapshot.aggregate, .idle)
        XCTAssertEqual(model.displaySnapshot.sessions, [])
        XCTAssertNotNil(model.runtimeSignalAgentUnavailableHint)
    }
    ...
    func testActivityPresentationCurrentLimitIncludesAllSupportedEntrypoints() throws {
        ...
        XCTAssertFalse(visible.contains { $0.sessionID == "platform-presence:codex-cli" })
    }
    ...
    @MainActor
    func testLowPowerModeDefaultsOnPersistsAndKeepsNewZealandModeIndependent() {
        let defaults = UserDefaults.standard
        let keys = [
            "isLowPowerModeEnabled",
            "isNewZealandTrafficLightModeEnabled"
        ...
        XCTAssertFalse(defaults.bool(forKey: "isLowPowerModeEnabled"))
        XCTAssertTrue(model.isNewZealandTrafficLightModeEnabled)
        XCTAssertTrue(defaults.bool(forKey: "isNewZealandTrafficLightModeEnabled"))
    ...
    @MainActor
    func testTaskStatusBubbleSoundsDefaultAndPersistIndependently() {
        let defaults = UserDefaults.standard
        let keys = [
        ...
        let model = MenuBarStatusModel()
        XCTAssertEqual(model.completionBubbleCompletionSound, .glass)
        XCTAssertEqual(model.completionBubblePermissionSound, .ping)

        model.setCompletionBubbleCompletionSound(.off)
        model.setCompletionBubblePermissionSound(.submarine)

        XCTAssertEqual(model.completionBubbleCompletionSound, .off)
        XCTAssertEqual(model.completionBubblePermissionSound, .submarine)
        XCTAssertEqual(defaults.string(forKey: "completionBubbleCompletionSound"), TaskStatusBubbleSound.off.rawValue)
        XCTAssertEqual(defaults.string(forKey: "completionBubblePermissionSound"), TaskStatusBubbleSound.submarine.rawValue)
    }
    ...
    func testRuntimeSignalAgentScopesMatchSupportedSources() throws {
        let now = Date()
        let sessions: [(RuntimeSignalAgentScope, SessionStatus)] = [
            (
                .codexDesktop,
                SessionStatus(
                    sessionID: "codex-desktop:thread",
        ...
        XCTAssertFalse(RuntimeSignalAgentScope.codexDesktop.matches(session: cliSession))
        XCTAssertFalse(RuntimeSignalAgentScope.codexVSCode.matches(session: cliSession))
    }

    @MainActor
    func testRuntimeSignalAgentScopesExposeSingleClaudeDesktopChoice() {
        XCTAssertFalse(RuntimeSignalAgentScope.selectableCases.contains(.claudeDesktop))
        XCTAssertTrue(RuntimeSignalAgentScope.selectableCases.contains(.claudeCode))
        ...
        XCTAssertTrue(RuntimeSignalAgentScope.claudeCode.matches(session: claudeDesktopSession))

        let model = MenuBarStatusModel()
        model.setAppLanguage(.zhHans)
        XCTAssertEqual(model.displayName(for: RuntimeSignalAgentScope.claudeCode), "Claude 桌面版")
    }
    ...
    private func makeTemporaryStore(
        ...
        let directory = URL(fileURLWithPath: NSTemporaryDirectory(), isDirectory: true)
            .appendingPathComponent("codex-agent-runtime-signal-light-tests-\(UUID().uuidString)", isDirectory: true)
    ...
    private func makeTemporaryCodexSessionsRoot() throws -> (sessionsRoot: URL, directory: URL) {
        let directory = URL(fileURLWithPath: NSTemporaryDirectory(), isDirectory: true)
            .appendingPathComponent("codex-agent-runtime-signal-light-codex-sessions-\(UUID().uuidString)", isDirectory: true)
    ...
    private func clearRuntimeSignalSelectionDefaults() -> [(String, Any?)] {
        let defaults = UserDefaults.standard
        let keys = [
            "runtimeSignalAgentScope",
            "runtimeSignalAgentScopes",
    ...
    private func restoreRuntimeSignalSelectionDefaults(_ savedValues: [(String, Any?)]) {
        let defaults = UserDefaults.standard
