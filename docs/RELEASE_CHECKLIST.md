# codex-agent-runtime-signal Release Checklist
...
- `./scripts/codex-agent-runtime-signal status --json` parses
...
- bundled `codex-agent-runtime-signal-run` preserves failed command exit codes and marks blocked
...
- uninstall removes the app, launch-agent plist, codex-agent-runtime-signal hooks, and configured state directory while preserving unrelated hooks
...
- Confirm `./script/build_and_run.sh --ui-verify` exits successfully; the script now checks for a real on-screen `codex-agent-runtime-signal` settings window.
...
- Open `连接` and click `版本`; Finder should select `dist/CodexAgentRuntimeSignal-release-manifest.json` when a release manifest is present, or the bundled `CodexAgentRuntimeSignal-release-info.json` after DMG installation.
...
- Use `./scripts/codex-agent-runtime-signal stale --session smoke --agent script --event StaleCheck` and confirm stale uses the gray/yellow stale display.
...
- In the app, open `连接` and click `复制接入命令`; pasteboard should contain a `generic-codex-agent-runtime-signal-hook` command that other agents can call with JSON events.
...
- If a previous codex-agent-runtime-signal path was configured, confirm `安装连接` migrates that wrapper path instead of adding duplicate codex-agent-runtime-signal hooks.
...
- For release zip startup, confirm `./script/install_app.sh --no-open` installs the existing `dist/CodexAgentRuntimeSignal.app` without rebuilding.
- For scripted DMG startup, run `./script/install_app.sh --dmg dist/CodexAgentRuntimeSignal.dmg`.
...
- Run `./script/uninstall_app.sh --remove-hooks` when you also want to remove codex-agent-runtime-signal hooks from Codex and Claude Code configs.
...
- Package with `CODEX_AGENT_RUNTIME_SIGNAL_CODE_SIGN_IDENTITY="Developer ID Application: ..."` so the app is signed with hardened runtime and timestamp.
- Submit with `CODEX_AGENT_RUNTIME_SIGNAL_NOTARY_PROFILE=<profile> ./script/notarize_release.sh --submit`.
