...
- `CodexAgentRuntimeSignal.zip`
- `CodexAgentRuntimeSignal.dmg`
- `appcast.xml`
- `CodexAgentRuntimeSignal-release-manifest.json`
- `CodexAgentRuntimeSignal-SHA256SUMS.txt`

`appcast.xml` is the Sparkle update feed used by installed apps. The app points to:

```text
https://github.com/guan-ops/codex-agent-runtime-signal/releases/latest/download/appcast.xml
```
...
.build/artifacts/sparkle/Sparkle/bin/generate_keys \
  --account com.codexagentruntimesignal.app \
  -x /tmp/CodexAgentRuntimeSignal-sparkle-private-key.txt
```
