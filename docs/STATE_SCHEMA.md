...
codex-agent-runtime-signal 默认读取和写入：
...
export RUNTIME_SIGNAL_SESSION_TTL_SECONDS=1800
```
...
`codex-agent-runtime-signal status --json` prints a machine-readable snapshot. Commands that update state, such as `codex-agent-runtime-signal working --session job --json`, can also use the same output format.
...
```json
{
  ...
  "action": "不用处理",
  "state_file": "/tmp/codex-agent-runtime-signal/status.json",
  "updated_at": "2026-05-28T03:45:00Z",
  "sessions": [
    {
      "session_id": "codex-main",
