...
PROFILE="${CODEX_AGENT_RUNTIME_SIGNAL_NOTARY_PROFILE:-${NOTARYTOOL_PROFILE:-}}"
...
Environment:
  CODEX_AGENT_RUNTIME_SIGNAL_CODE_SIGN_IDENTITY   Developer ID Application identity used by package_app.sh.
  CODEX_AGENT_RUNTIME_SIGNAL_NOTARY_PROFILE       xcrun notarytool keychain profile.

Typical flow:
  CODEX_AGENT_RUNTIME_SIGNAL_CODE_SIGN_IDENTITY="Developer ID Application: ..." ./script/package_release.sh
  CODEX_AGENT_RUNTIME_SIGNAL_NOTARY_PROFILE="codex-agent-runtime-signal-light" ./script/notarize_release.sh --submit
EOF
      exit 0
      ;;
    *)
...
print_readiness() {
  ...
  echo "codex-agent-runtime-signal notarization readiness"
  echo "dmg: $DMG_PATH"
  ...
  if [[ -n "$PROFILE" ]]; then
    ...
    echo "[missing] CODEX_AGENT_RUNTIME_SIGNAL_NOTARY_PROFILE or --profile"
  fi
...
submit_notarization() {
  ...
  if [[ -z "$PROFILE" ]]; then
    echo "notarize_release: missing CODEX_AGENT_RUNTIME_SIGNAL_NOTARY_PROFILE or --profile" >&2
    exit 1
  fi
  if [[ ! -f "$DMG_PATH" ]]; then
