...
swift_tool build -c release --product codex-agent-runtime-signal-icon-preview >/dev/null
PREVIEW_BINARY="$(swift_tool build -c release --show-bin-path)/codex-agent-runtime-signal-icon-preview"
mkdir -p "$ROOT_DIR/dist/bin"
cp "$PREVIEW_BINARY" "$ROOT_DIR/dist/bin/codex-agent-runtime-signal-icon-preview"
chmod +x "$ROOT_DIR/dist/bin/codex-agent-runtime-signal-icon-preview"
"$ROOT_DIR/dist/bin/codex-agent-runtime-signal-icon-preview" "$ROOT_DIR/dist/status-icon-preview" >/dev/null
...
for root_file in LICENSE NOTICE ASSET_LICENSES.md TRADEMARKS.md CHANGELOG.md VERSION; do
...
cp "$ROOT_DIR/dist/bin/codex-agent-runtime-signal" "$PAYLOAD_DIR/dist/bin/codex-agent-runtime-signal"
cp "$ROOT_DIR/dist/bin/codex-agent-runtime-signal-icon-preview" "$PAYLOAD_DIR/dist/bin/codex-agent-runtime-signal-icon-preview"
...
chmod +x "$PAYLOAD_DIR/dist/bin/codex-agent-runtime-signal" \
  "$PAYLOAD_DIR/dist/bin/codex-agent-runtime-signal-icon-preview" \
  "$PAYLOAD_DIR/scripts/codex-agent-runtime-signal" \
