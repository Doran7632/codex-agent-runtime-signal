...
APP_DISPLAY_NAME="codex-agent-runtime-signal"
BUNDLE_ID="com.codexagentruntimesignal.app"
MIN_SYSTEM_VERSION="14.0"
XCODE_DEVELOPER_DIR="/Applications/Xcode.app/Contents/Developer"
SIGN_IDENTITY="${CODEX_AGENT_RUNTIME_SIGNAL_CODE_SIGN_IDENTITY:-${CODE_SIGN_IDENTITY:-}}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://github.com/guan-ops/codex-agent-runtime-signal/releases/latest/download/appcast.xml}"
...
CLI_BUILD_ARGS=(--product codex-agent-runtime-signal)
...
CLI_BINARY="$CLI_BIN_DIR/codex-agent-runtime-signal"
...
cp "$CLI_BINARY" "$APP_RESOURCES/dist/bin/codex-agent-runtime-signal"
...
cp "$ROOT_DIR/scripts/codex-agent-runtime-signal" "$APP_RESOURCES/scripts/codex-agent-runtime-signal"
cp "$ROOT_DIR/scripts/codex-agent-runtime-signal-run" "$APP_RESOURCES/scripts/codex-agent-runtime-signal-run"
...
cp "$ROOT_DIR/scripts/generic-codex-agent-runtime-signal-hook" "$APP_RESOURCES/scripts/generic-codex-agent-runtime-signal-hook"
for audio_resource in "$ROOT_DIR"/Sources/CodexAgentRuntimeSignal/Resources/*.{m4a,wav}; do
...
chmod +x "$APP_BINARY"
chmod +x "$APP_RESOURCES/dist/bin/codex-agent-runtime-signal" \
  "$APP_RESOURCES/script/export_diagnostics.sh" \
  "$APP_RESOURCES/script/install_hooks.py" \
  "$APP_RESOURCES/scripts/codex-agent-runtime-signal" \
  "$APP_RESOURCES/scripts/codex-agent-runtime-signal-run" \
  "$APP_RESOURCES/scripts/codex-signal-hook" \
  "$APP_RESOURCES/scripts/claude-code-signal-hook" \
  "$APP_RESOURCES/scripts/generic-codex-agent-runtime-signal-hook"
...
app_name = sys.argv[3]
signing_identity = os.environ.get("CODEX_AGENT_RUNTIME_SIGNAL_CODE_SIGN_IDENTITY") or os.environ.get("CODE_SIGN_IDENTITY")
notary_profile = os.environ.get("CODEX_AGENT_RUNTIME_SIGNAL_NOTARY_PROFILE") or os.environ.get("NOTARYTOOL_PROFILE")
...
if [[ -n "$SIGN_IDENTITY" ]]; then
  ...
  codesign --force --options runtime --timestamp --sign "$SIGN_IDENTITY" "$APP_RESOURCES/dist/bin/codex-agent-runtime-signal" >/dev/null
