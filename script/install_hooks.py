#!/usr/bin/env python3
"""Safely merge codex-agent-runtime-signal hooks into local Codex and Claude configs."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Merge codex-agent-runtime-signal hook commands into Codex and Claude Code JSON configs."
    )
    parser.add_argument(
        "--target",
    ...
    parser.add_argument(
        "--remove",
        action="store_true",
        help="Remove codex-agent-runtime-signal hook commands instead of installing them. Combine with --install to write.",
    )
    parser.add_argument(
        "--home",
        type=Path,
...
def command_points_to_runtime_signal_wrapper(command: str, spec: TargetSpec) -> bool:
    try:
...
def remove_stale_runtime_signal_commands(
    blocks: list[Any],
    *,
    spec: TargetSpec,
    current_command: str,
    ...
    for block in blocks:
        ...
        for item in hooks:
            ...
            if (
                ...
                and command_points_to_runtime_signal_wrapper(command, spec)
            ):
                migrated = True
                continue
...
def merge_hooks(spec: TargetSpec) -> MergeResult:
    ...
    for event in spec.events:
        ...
        blocks, migrated = remove_stale_runtime_signal_commands(
            blocks,
            spec=spec,
    ...
    for event, blocks in list(hooks.items()):
        ...
        if not isinstance(blocks, list):
            continue
        filtered, removed = remove_stale_runtime_signal_commands(
            blocks,
            spec=spec,
            current_command="",
        )
    ...
    for event, blocks in list(hooks.items()):
        ...
        for block in blocks:
            ...
            for item in block_hooks:
                command = item.get("command") if isinstance(item, dict) else None
                if isinstance(command, str) and command_points_to_runtime_signal_wrapper(command, spec):
...
def print_claude_runtime_diagnostics(home: Path) -> None:
    ...
    if "Claude Code requires a Pro or Max subscription" in log_text:
        ...
        print("  effect: normal Claude Desktop chat does not emit Claude Code hook events, so codex-agent-runtime-signal can only show app presence until Claude Code/local-agent sessions can start.")
