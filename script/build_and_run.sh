...
Builds codex-agent-runtime-signal, launches the app bundle, and optionally verifies UI/runtime health.
EOF
}
...
run_app_binary() {
  "$APP_BINARY" "$@" >/tmp/codex-agent-runtime-signal-light-ui-verify.log 2>&1 &
  APP_BINARY_PID=$!
...
let hasDebugWindow = windows.contains { window in
    ...
    let isCodexAgentRuntimeSignal = owner == "CodexAgentRuntimeSignal" || owner == "codex-agent-runtime-signal"
    return isCodexAgentRuntimeSignal && name == "codex-agent-runtime-signal"
}
...
case "$MODE" in
  ...
  --status-item-verify|status-item-verify)
    HEALTH_DIR="$(mktemp -d "${TMPDIR:-/tmp}/codex-agent-runtime-signal-status-item-health.XXXXXX")"
