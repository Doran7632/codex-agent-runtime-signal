...
Options:
  --dmg <path>   DMG to verify. Defaults to dist/$RELEASE_BASENAME.dmg.
...
[[ -f "$MOUNT_DIR/Read Me.txt" ]] || die "DMG is missing Read Me.txt"
pass "DMG layout contains app, Applications shortcut, and text readme"
...
[[ -x "$CLI_WRAPPER" ]] || die "bundled codex-agent-runtime-signal wrapper missing"
[[ -x "$RUN_WRAPPER" ]] || die "bundled codex-agent-runtime-signal-run wrapper missing"
[[ -x "$GENERIC_WRAPPER" ]] || die "bundled generic agent hook wrapper missing"
[[ -x "$CLI_BIN" ]] || die "bundled codex-agent-runtime-signal binary missing"
...
CODEX_AGENT_RUNTIME_SIGNAL_STATE_FILE="$TMP_STATE" "$CLI_WRAPPER" blocked --session install-red --agent release --event InstallVerifier >/dev/null
CODEX_AGENT_RUNTIME_SIGNAL_STATE_FILE="$TMP_STATE" "$CLI_WRAPPER" working --session install-active --agent release --event InstallVerifier --json >"$TMP_ROOT/status.json"
...
set +e
CODEX_AGENT_RUNTIME_SIGNAL_STATE_FILE="$TMP_RUN_STATE" "$RUN_WRAPPER" --session install-run-fail --agent release -- /bin/sh -c "exit 8" >/dev/null
run_status=$?
set -e
[[ "$run_status" -eq 8 ]] || die "bundled codex-agent-runtime-signal-run did not preserve wrapped exit code"
...
PY
pass "bundled codex-agent-runtime-signal-run preserves exit code and marks blocked"
