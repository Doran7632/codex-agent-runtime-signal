...
verify_status_json() {
  local state_file
  state_file="$(mktemp "${TMPDIR:-/tmp}/codex-agent-runtime-signal-release-json.XXXXXX")"
  CODEX_AGENT_RUNTIME_SIGNAL_STATE_FILE="$state_file" "$ROOT_DIR/scripts/codex-agent-runtime-signal" status --json >/tmp/codex-agent-runtime-signal-release-status.json
  /usr/bin/python3 - /tmp/codex-agent-runtime-signal-release-status.json <<'PY'
import json
import sys
...
PY
  rm -f "$state_file" /tmp/codex-agent-runtime-signal-release-status.json
}
...
printf "codex-agent-runtime-signal release gate\n"
printf "root: %s\n" "$ROOT_DIR"
