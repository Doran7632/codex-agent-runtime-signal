...
APP_NAME="CodexAgentRuntimeSignal"
RELEASE_BASENAME="CodexAgentRuntimeSignal"
BUNDLE_ID="com.codexagentruntimesignal.app"
STATE_DIR="${CODEX_AGENT_RUNTIME_SIGNAL_STATE_DIR:-/tmp/codex-agent-runtime-signal}"
STATE_FILE="${CODEX_AGENT_RUNTIME_SIGNAL_STATE_FILE:-$STATE_DIR/status.json}"
APP_BUNDLE="$ROOT_DIR/dist/$APP_NAME.app"
CLI_BIN="$ROOT_DIR/dist/bin/codex-agent-runtime-signal"
...
FAILED=0
WARNED=0
DOCTOR_TMP_DIR="$(mktemp -d "${TMPDIR:-/tmp}/codex-agent-runtime-signal-doctor.XXXXXX")"
...
echo "codex-agent-runtime-signal doctor"
echo "root: $ROOT_DIR"

[[ -f "$ROOT_DIR/Package.swift" ]] && pass "Package.swift found" || fail "Package.swift missing"

if [[ "$MODE" == "full" ]]; then
  run_check "core checks pass" swift_tool run codex-agent-runtime-signal-checks
  ...
  if swift_tool run codex-agent-runtime-signal-icon-preview "$ROOT_DIR/dist/status-icon-preview" >"$DOCTOR_OUT" 2>"$DOCTOR_ERR"; then
...
with zipfile.ZipFile(archive) as package:
    ...
    required_suffixes = [
        "manifest.json",
        "commands/codex-agent-runtime-signal-status.txt",
        "commands/codex-agent-runtime-signal-status-json.txt",
...
PY
  ...
  if "$ROOT_DIR/script/notarize_release.sh" --readiness >"$DOCTOR_OUT" 2>"$DOCTOR_ERR"; then
    if grep -q "codex-agent-runtime-signal notarization readiness" "$DOCTOR_OUT"; then
...
if [[ -x "$ROOT_DIR/scripts/codex-agent-runtime-signal" ]]; then
  pass "codex-agent-runtime-signal wrapper executable"
else
  fail "scripts/codex-agent-runtime-signal is not executable"
fi

if [[ -x "$ROOT_DIR/scripts/codex-agent-runtime-signal-run" ]]; then
  pass "codex-agent-runtime-signal-run wrapper executable"
else
  fail "scripts/codex-agent-runtime-signal-run is not executable"
fi

if [[ -x "$ROOT_DIR/scripts/codex-signal-hook" \
  && -x "$ROOT_DIR/scripts/claude-code-signal-hook" \
  && -x "$ROOT_DIR/scripts/generic-codex-agent-runtime-signal-hook" ]]; then
...
if CODEX_AGENT_RUNTIME_SIGNAL_STATE_FILE="$TMP_STATE" "$ROOT_DIR/scripts/codex-agent-runtime-signal" permission --session doctor --agent doctor --event PermissionRequest >"$DOCTOR_OUT" 2>"$DOCTOR_ERR"; then
...
PY
  ...
  fail "codex-agent-runtime-signal CLI failed"
  sed -n '1,5p' "$DOCTOR_ERR"
fi

TMP_JSON_STATE="$(mktemp -d)/status.json"
if CODEX_AGENT_RUNTIME_SIGNAL_STATE_FILE="$TMP_JSON_STATE" "$ROOT_DIR/scripts/codex-agent-runtime-signal" working --session doctor-json --agent doctor --event JSONStatus --json >"$DOCTOR_OUT" 2>"$DOCTOR_ERR"; then
  if /usr/bin/python3 - "$DOCTOR_OUT" <<'PY'
import json
import sys
from pathlib import Path
...
PY
  ...
  fail "codex-agent-runtime-signal --json output failed"
  sed -n '1,5p' "$DOCTOR_ERR"
fi

TMP_PRIORITY_STATE="$(mktemp -d)/status.json"
if CODEX_AGENT_RUNTIME_SIGNAL_STATE_FILE="$TMP_PRIORITY_STATE" "$ROOT_DIR/scripts/codex-agent-runtime-signal" permission --session doctor-permission --agent doctor --event PermissionRequest >"$DOCTOR_OUT" 2>"$DOCTOR_ERR" \
  && CODEX_AGENT_RUNTIME_SIGNAL_STATE_FILE="$TMP_PRIORITY_STATE" "$ROOT_DIR/scripts/codex-agent-runtime-signal" working --json >"$DOCTOR_OUT" 2>"$DOCTOR_ERR"; then
  if /usr/bin/python3 - "$DOCTOR_OUT" <<'PY'
import json
import sys
from pathlib import Path
...
if CODEX_AGENT_RUNTIME_SIGNAL_STATE_FILE="$TMP_RESUME_STATE" "$ROOT_DIR/scripts/codex-agent-runtime-signal" off --session doctor-off --agent doctor --event Pause >"$DOCTOR_OUT" 2>"$DOCTOR_ERR" \
  && CODEX_AGENT_RUNTIME_SIGNAL_STATE_FILE="$TMP_RESUME_STATE" "$ROOT_DIR/scripts/codex-agent-runtime-signal" working --session doctor-resume --agent doctor --event Resume --json >"$DOCTOR_OUT" 2>"$DOCTOR_ERR"; then
  if /usr/bin/python3 - "$DOCTOR_OUT" <<'PY'
import json
import sys
from pathlib import Path
...
for signal in idle thinking working tool_done subagent_start subagent_stop attention notification done permission permission_request blocked failure error exception max_tokens stale session_start session_end turn_end off pause paused; do
  if ! CODEX_AGENT_RUNTIME_SIGNAL_STATE_FILE="$TMP_SIGNAL_STATE" "$ROOT_DIR/scripts/codex-agent-runtime-signal" "$signal" --session "doctor-$signal" --agent doctor --event "$signal" >"$DOCTOR_OUT" 2>"$DOCTOR_ERR"; then
...
if CODEX_AGENT_RUNTIME_SIGNAL_STATE_FILE="$TMP_COMPLETED_STATE" CODEX_AGENT_RUNTIME_SIGNAL_COMPLETED_TTL_SECONDS=0.01 "$ROOT_DIR/scripts/codex-agent-runtime-signal" done --session doctor-done --agent doctor --event Done >"$DOCTOR_OUT" 2>"$DOCTOR_ERR" \
  && sleep 0.03 \
  && CODEX_AGENT_RUNTIME_SIGNAL_STATE_FILE="$TMP_COMPLETED_STATE" CODEX_AGENT_RUNTIME_SIGNAL_COMPLETED_TTL_SECONDS=0.01 "$ROOT_DIR/scripts/codex-agent-runtime-signal" status --json >"$DOCTOR_OUT" 2>"$DOCTOR_ERR"; then
...
if CODEX_AGENT_RUNTIME_SIGNAL_STATE_FILE="$(mktemp -d)/status.json" "$ROOT_DIR/scripts/codex-agent-runtime-signal" definitely_unknown_signal >"$DOCTOR_OUT" 2>"$DOCTOR_ERR"; then
...
if CODEX_AGENT_RUNTIME_SIGNAL_STATE_FILE="$TMP_BAD_ARGS_STATE" "$ROOT_DIR/scripts/codex-agent-runtime-signal" working --session >"$DOCTOR_OUT" 2>"$DOCTOR_ERR"; then
...
if CODEX_AGENT_RUNTIME_SIGNAL_STATE_FILE="$TMP_UNKNOWN_OPTION_STATE" "$ROOT_DIR/scripts/codex-agent-runtime-signal" working --sesion typo >"$DOCTOR_OUT" 2>"$DOCTOR_ERR"; then
...
if printf '{"event":"PermissionRequest","session_id":"doctor-codex"}' \
  | CODEX_AGENT_RUNTIME_SIGNAL_STATE_FILE="$TMP_CODEX_STATE" "$ROOT_DIR/scripts/codex-signal-hook" >"$DOCTOR_OUT" 2>"$DOCTOR_ERR"; then
...
if printf '{"event":"Notification","session_id":"doctor-claude"}' \
  | CODEX_AGENT_RUNTIME_SIGNAL_STATE_FILE="$TMP_CLAUDE_STATE" "$ROOT_DIR/scripts/claude-code-signal-hook" >"$DOCTOR_OUT" 2>"$DOCTOR_ERR"; then
...
if printf '{"event":"post_tool_use_failure","sessionId":"doctor-normalized-claude"}' \
  | CODEX_AGENT_RUNTIME_SIGNAL_STATE_FILE="$TMP_NORMALIZED_EVENT_STATE" "$ROOT_DIR/scripts/claude-code-signal-hook" >"$DOCTOR_OUT" 2>"$DOCTOR_ERR" \
  && printf '{"sessionId":"doctor-normalized-codex"}' \
  | CODEX_AGENT_RUNTIME_SIGNAL_STATE_FILE="$TMP_NORMALIZED_EVENT_STATE" "$ROOT_DIR/scripts/codex-signal-hook" " pre_tool_use " >"$DOCTOR_OUT" 2>"$DOCTOR_ERR"; then
...
if printf '{"event":"Stop","session_id":"doctor-codex-stop"}' \
  | CODEX_AGENT_RUNTIME_SIGNAL_STATE_FILE="$TMP_STOP_STATE" "$ROOT_DIR/scripts/codex-signal-hook" >"$DOCTOR_OUT" 2>"$DOCTOR_ERR" \
  && printf '{"event":"Stop","session_id":"doctor-claude-stop"}' \
  | CODEX_AGENT_RUNTIME_SIGNAL_STATE_FILE="$TMP_STOP_STATE" "$ROOT_DIR/scripts/claude-code-signal-hook" >"$DOCTOR_OUT" 2>"$DOCTOR_ERR"; then
...
if printf '{"event":"Stop","session_id":"doctor-stop-end"}' \
  | CODEX_AGENT_RUNTIME_SIGNAL_STATE_FILE="$TMP_STOP_SESSION_END_STATE" "$ROOT_DIR/scripts/codex-signal-hook" >"$DOCTOR_OUT" 2>"$DOCTOR_ERR" \
  && printf '{"event":"SessionEnd","session_id":"doctor-stop-end"}' \
  | CODEX_AGENT_RUNTIME_SIGNAL_STATE_FILE="$TMP_STOP_SESSION_END_STATE" "$ROOT_DIR/scripts/codex-signal-hook" >"$DOCTOR_OUT" 2>"$DOCTOR_ERR"; then
...
if CODEX_AGENT_RUNTIME_SIGNAL_STATE_FILE="$TMP_STOP_PRESERVE_STATE" "$ROOT_DIR/scripts/codex-agent-runtime-signal" permission --session doctor-stop-preserve --agent doctor --event PermissionRequest >"$DOCTOR_OUT" 2>"$DOCTOR_ERR" \
  && printf '{"event":"Stop","session_id":"doctor-stop-preserve"}' \
  | CODEX_AGENT_RUNTIME_SIGNAL_STATE_FILE="$TMP_STOP_PRESERVE_STATE" "$ROOT_DIR/scripts/codex-signal-hook" >"$DOCTOR_OUT" 2>"$DOCTOR_ERR"; then
...
if printf '{"event":"PostToolUse","sessionId":"doctor-claude-fail","exitStatus":1}' \
  | CODEX_AGENT_RUNTIME_SIGNAL_STATE_FILE="$TMP_CLAUDE_FAILURE_STATE" "$ROOT_DIR/scripts/claude-code-signal-hook" >"$DOCTOR_OUT" 2>"$DOCTOR_ERR"; then
...
if printf '{"event":" stop ","sessionId":"doctor-claude-max-tokens","stopReason":" max-tokens "}' \
  | CODEX_AGENT_RUNTIME_SIGNAL_STATE_FILE="$TMP_CLAUDE_STOP_REASON_STATE" "$ROOT_DIR/scripts/claude-code-signal-hook" >"$DOCTOR_OUT" 2>"$DOCTOR_ERR" \
  && printf '{"event":"Stop","sessionId":"doctor-claude-stop-error","stopReason":" tool error "}' \
  | CODEX_AGENT_RUNTIME_SIGNAL_STATE_FILE="$TMP_CLAUDE_STOP_REASON_STATE" "$ROOT_DIR/scripts/claude-code-signal-hook" >"$DOCTOR_OUT" 2>"$DOCTOR_ERR"; then
...
if CODEX_AGENT_RUNTIME_SIGNAL_STATE_FILE="$TMP_RUNNER_STATE" "$ROOT_DIR/scripts/codex-agent-runtime-signal-run" --session runner-ok --agent script -- /bin/sh -c "exit 0" >"$DOCTOR_OUT" 2>"$DOCTOR_ERR"; then
...
PY
  then
    pass "codex-agent-runtime-signal-run marks successful commands done"
  else
    fail "codex-agent-runtime-signal-run success state check failed"
  fi
else
  fail "codex-agent-runtime-signal-run failed for successful command"
  sed -n '1,5p' "$DOCTOR_ERR"
fi

TMP_RUNNER_FAIL_STATE="$(mktemp -d)/status.json"
CODEX_AGENT_RUNTIME_SIGNAL_STATE_FILE="$TMP_RUNNER_FAIL_STATE" "$ROOT_DIR/scripts/codex-agent-runtime-signal-run" --session runner-fail --agent script -- /bin/sh -c "exit 7" >"$DOCTOR_OUT" 2>"$DOCTOR_ERR"
RUNNER_FAIL_STATUS=$?
...
PY
  then
    pass "codex-agent-runtime-signal-run preserves failure exit code and marks blocked"
  else
    fail "codex-agent-runtime-signal-run failure state check failed"
  fi
else
  fail "codex-agent-runtime-signal-run did not preserve failure exit code"
  sed -n '1,5p' "$DOCTOR_ERR"
fi
...
codex_hooks_file_mentions_runtime_signal() {
  local hooks_file="$1"
...
if [[ -f "$CODEX_HOOKS_FILE" ]]; then
  ...
  elif codex_hooks_file_mentions_runtime_signal "$CODEX_HOOKS_FILE" >"$DOCTOR_OUT" 2>"$DOCTOR_ERR"; then
    warn "Codex user hooks contain stale/incomplete codex-agent-runtime-signal entries; run ./script/install_hooks.py --target codex --codex-scope user --remove --install"
  else
...
if [[ -x "$APP_BUNDLE/Contents/MacOS/$APP_NAME" ]]; then
  ...
  if [[ -x "$APP_RESOURCE_ROOT/dist/bin/codex-agent-runtime-signal" \
    && -x "$APP_RESOURCE_ROOT/script/export_diagnostics.sh" \
    && -x "$APP_RESOURCE_ROOT/script/install_hooks.py" \
    && -x "$APP_RESOURCE_ROOT/scripts/codex-agent-runtime-signal" \
    && -x "$APP_RESOURCE_ROOT/scripts/codex-agent-runtime-signal-run" \
    && -x "$APP_RESOURCE_ROOT/scripts/codex-signal-hook" \
    && -x "$APP_RESOURCE_ROOT/scripts/claude-code-signal-hook" \
    && -x "$APP_RESOURCE_ROOT/scripts/generic-codex-agent-runtime-signal-hook" \
  ...
  if CODEX_AGENT_RUNTIME_SIGNAL_STATE_FILE="$TMP_BUNDLE_STATE" "$APP_RESOURCE_ROOT/scripts/codex-agent-runtime-signal" working --session bundled-doctor --agent bundled --event BundledResource >"$DOCTOR_OUT" 2>"$DOCTOR_ERR"; then
    pass "bundled codex-agent-runtime-signal wrapper writes state"
  else
    fail "bundled codex-agent-runtime-signal wrapper failed"
    sed -n '1,5p' "$DOCTOR_ERR"
  fi
...
PY
  ...
  if CODEX_AGENT_RUNTIME_SIGNAL_STATE_FILE="$TMP_BUNDLE_PRIORITY_STATE" "$APP_RESOURCE_ROOT/scripts/codex-agent-runtime-signal" blocked --session bundled-red --agent bundled --event Failure >"$DOCTOR_OUT" 2>"$DOCTOR_ERR" \
    && CODEX_AGENT_RUNTIME_SIGNAL_STATE_FILE="$TMP_BUNDLE_PRIORITY_STATE" "$APP_RESOURCE_ROOT/scripts/codex-agent-runtime-signal" working --json >"$DOCTOR_OUT" 2>"$DOCTOR_ERR"; then
    if /usr/bin/python3 - "$DOCTOR_OUT" <<'PY'
import json
import sys
from pathlib import Path
...
PY
  ...
  CODEX_AGENT_RUNTIME_SIGNAL_STATE_FILE="$TMP_BUNDLE_RUNNER_STATE" "$APP_RESOURCE_ROOT/scripts/codex-agent-runtime-signal-run" --session bundled-runner-fail --agent bundled -- /bin/sh -c "exit 9" >"$DOCTOR_OUT" 2>"$DOCTOR_ERR"
...
PY
    then
      pass "bundled codex-agent-runtime-signal-run preserves failure exit code"
    else
      fail "bundled codex-agent-runtime-signal-run state check failed"
    fi
  else
    fail "bundled codex-agent-runtime-signal-run did not preserve failure exit code"
    sed -n '1,5p' "$DOCTOR_ERR"
  fi
...
for path, removed, keep in checks:
    text = path.read_text()
    if removed in text:
        raise SystemExit(f"codex-agent-runtime-signal hook was not removed from {path}")
    if keep not in text:
        raise SystemExit(f"unrelated hook was not preserved in {path}")
    if not list(path.parent.glob(path.name + ".bak-*")):
        raise SystemExit(f"backup was not created for {path}")
PY
    then
      pass "bundled hook installer removes codex-agent-runtime-signal hooks"
    else
  ...
  if CODEX_AGENT_RUNTIME_SIGNAL_APP_INSTALL_DIR="$TMP_INSTALL_ROOT/Applications" "$ROOT_DIR/script/install_app.sh" --source-app "$APP_BUNDLE" --no-open >"$DOCTOR_OUT" 2>"$DOCTOR_ERR"; then
...
assert manifest["app_name"] == "CodexAgentRuntimeSignal"
assert manifest["bundle_identifier"] == "com.codexagentruntimesignal.app"
...
PY
    ...
    if CODEX_AGENT_RUNTIME_SIGNAL_APP_INSTALL_DIR="$TMP_DMG_INSTALL_ROOT/Applications" "$ROOT_DIR/script/install_app.sh" --dmg "$DMG" --no-open >"$DOCTOR_OUT" 2>"$DOCTOR_ERR"; then
