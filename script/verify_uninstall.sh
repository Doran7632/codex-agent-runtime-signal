...
Install codex-agent-runtime-signal into a temporary HOME and Applications directory,
then verify uninstall_app.sh removes the app, launch-agent plist, codex-agent-runtime-signal
...
STATE_DIR="$TMP_ROOT/codex-agent-runtime-signal-state"
...
HOME="$TMP_HOME" \
CODEX_AGENT_RUNTIME_SIGNAL_APP_INSTALL_DIR="$INSTALL_DIR" \
...
HOME="$TMP_HOME" \
CODEX_AGENT_RUNTIME_SIGNAL_APP_INSTALL_DIR="$INSTALL_DIR" \
CODEX_AGENT_RUNTIME_SIGNAL_STATE_DIR="$STATE_DIR" \
...
PY
pass "uninstall removes codex-agent-runtime-signal hooks and preserves unrelated hooks"
