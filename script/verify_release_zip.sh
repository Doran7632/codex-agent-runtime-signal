...
[[ -x "$PAYLOAD/dist/bin/codex-agent-runtime-signal" ]] || die "packaged release CLI missing"
[[ -x "$PAYLOAD/dist/bin/codex-agent-runtime-signal-icon-preview" ]] || die "packaged icon preview CLI missing"
[[ -x "$PAYLOAD/scripts/codex-agent-runtime-signal-run" ]] || die "packaged codex-agent-runtime-signal-run wrapper missing"
[[ -x "$PAYLOAD/scripts/generic-codex-agent-runtime-signal-hook" ]] || die "packaged generic agent hook wrapper missing"
...
(
  cd "$PAYLOAD"
  CODEX_AGENT_RUNTIME_SIGNAL_APP_INSTALL_DIR="$INSTALL_ROOT" ./script/install_app.sh --no-open >"$TMP_ROOT/install.out"
)
...
CLI_WRAPPER="$APP_RESOURCES/scripts/codex-agent-runtime-signal"
RUN_WRAPPER="$APP_RESOURCES/scripts/codex-agent-runtime-signal-run"
GENERIC_WRAPPER="$APP_RESOURCES/scripts/generic-codex-agent-runtime-signal-hook"
CLI_BIN="$APP_RESOURCES/dist/bin/codex-agent-runtime-signal"
...
[[ -x "$CLI_WRAPPER" ]] || die "installed bundled codex-agent-runtime-signal wrapper missing"
[[ -x "$RUN_WRAPPER" ]] || die "installed bundled codex-agent-runtime-signal-run wrapper missing"
[[ -x "$GENERIC_WRAPPER" ]] || die "installed bundled generic agent hook wrapper missing"
[[ -x "$CLI_BIN" ]] || die "installed bundled codex-agent-runtime-signal binary missing"
...
if release_info.get("app_name") != "CodexAgentRuntimeSignal":
...
CODEX_AGENT_RUNTIME_SIGNAL_STATE_FILE="$TMP_STATE" "$PAYLOAD/scripts/codex-agent-runtime-signal" blocked --session zip-red --agent release-zip --event ZipVerifier >/dev/null
CODEX_AGENT_RUNTIME_SIGNAL_STATE_FILE="$TMP_STATE" "$CLI_WRAPPER" working --session zip-active --agent release-zip --event ZipVerifier --json >"$TMP_ROOT/status.json"
...
set +e
CODEX_AGENT_RUNTIME_SIGNAL_STATE_FILE="$TMP_RUN_STATE" "$RUN_WRAPPER" --session zip-run-fail --agent release-zip -- /bin/sh -c "exit 8" >/dev/null
run_status=$?
set -e
[[ "$run_status" -eq 8 ]] || die "release zip codex-agent-runtime-signal-run did not preserve wrapped exit code"
...
if data["aggregate"] != "blocked":
    raise SystemExit("codex-agent-runtime-signal-run did not mark aggregate blocked")
if sessions["zip-run-fail"]["signal"] != "blocked":
    raise SystemExit("codex-agent-runtime-signal-run did not mark failed session blocked")
if sessions["zip-run-fail"]["last_event"] != "CommandFailed:8":
    raise SystemExit("codex-agent-runtime-signal-run did not record wrapped exit code")
PY
pass "release zip codex-agent-runtime-signal-run preserves exit code and marks blocked"
...
with zipfile.ZipFile(archive) as package:
    ...
    if not any(name.endswith("commands/codex-agent-runtime-signal-status-json.txt") for name in names):
