...
INSTALL_DIR="${CODEX_AGENT_RUNTIME_SIGNAL_APP_INSTALL_DIR:-$HOME/Applications}"
...
HOOK_PROJECT_ROOT="${CODEX_AGENT_RUNTIME_SIGNAL_CODEX_PROJECT_ROOT:-$ROOT_DIR}"
PURGE_STATE=0
REMOVE_HOOKS=0
KILL_RUNNING_APP=1
RUN_LAUNCHCTL=1
...
Uninstall codex-agent-runtime-signal from the current user's Applications directory.

Options:
  --remove-hooks  Remove codex-agent-runtime-signal hooks from Codex and Claude config.
  --purge-state   Remove the state directory. Defaults to $STATE_DIR.
  --no-kill       Do not terminate running CodexAgentRuntimeSignal processes.
...
validate_state_dir_for_purge() {
  ...
  case "$target" in
    *codex-agent-runtime-signal*|*RuntimeSignal*|*runtime_signal*)
      ;;
    *)
...
if [[ "$REMOVE_HOOKS" -eq 1 ]]; then
  echo "codex-agent-runtime-signal hooks: removal requested"
fi
if [[ "$PURGE_STATE" -eq 1 ]]; then
